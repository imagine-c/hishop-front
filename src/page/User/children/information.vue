<template>
  <div>
    <y-shelf title="账户资料">
      <div slot="content">
        <div class="avatar-box">
          ...
          <div class="r-box">
            <h3 style="margin-left: 13px;">邮箱：{{userInfo.info.email}}</h3>
            <y-button text="修改邮箱" classStyle="main-btn" style="margin-left: 66px;;" @btnClick="changePopup(userInfo.info.email)"></y-button>
          </div>
        </div>
    ...
    <y-popup :open="popupOpen" @close='popupOpen=false' title='修改邮箱'>
      <div slot="content" class="md" :data-id="userId">
        <div>
          <input type="email" placeholder="邮箱" v-model="msg.email">
        </div>
        <y-button text='修改'
                  style="width: 100%;height: 50px;line-height: 48px"
                  :classStyle="msg.email?'main-btn':'disabled-btn'"
                  @btnClick="update({userId:userId,email:msg.email})">
        </y-button>
      </div>
    </y-popup>
  </div>
...
<script>
  import { emailUpdate } from '/api/goods'
  ...
  export default {
    ...
    methods: {
      ...
      update (p) {
        if (this.isEmail(p.email)) {
          emailUpdate(p).then(res => {
            if (res.success === true) {
