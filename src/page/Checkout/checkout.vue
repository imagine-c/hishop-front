        ...
        <div slot="content">
          <ul class="address-item-list clearfix">
            <li v-for="(item,i) in addList"
                ...
                @click="chooseAddress(item.addressId, item.userName, item.tel, item.provinceName, item.cityName, item.areaName, item.streetName, item.detailed)">
           <span v-if="addressId === item.addressId" class="pa">
             ...
             <path
              ...
              <p class="street-name ellipsis">收货地址: {{item.provinceName}}{{item.cityName}}{{item.areaName}}{{item.streetName}}{{item.detailed}}</p>
              <p>手机号码: {{item.tel}}</p>
...
<script>
  import { getCartList, addressList, addressUpdate, addressAdd, addressDel, productDet, submitOrder, getAddressJson } from '/api/goods'
  ...
  export default {
    data () {
      return {
        ...
        submitOrder: '提交订单',
        options: [],
        selectedOptions: ['120000', '120100', '120101']
      }
    },
    computed: {
      btnHighlight () {
    ...
    methods: {
      ...
      _addressList () {
        addressList({userId: this.userId}).then(res => {
          ...
          if (data.length) {
            ...
            this.tel = data[0].tel
            this.provinceName = data[0].provinceName
      ...
      // 选择地址
      chooseAddress (addressId, userName, tel, provinceName, cityName, areaName, streetName, detailed) {
